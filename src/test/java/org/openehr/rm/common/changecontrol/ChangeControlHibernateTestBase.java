...
public abstract class ChangeControlHibernateTestBase extends AbstractHibernateTest {
	...
	protected OriginalVersion<Locatable> originalVersion(Locatable data,
		...
		if (isMerged) {
			...
			return new OriginalVersion<Locatable>(uid, null, data, codedText,
					audit1, lr, null, otherUids, null, TestTerminologyService.getInstance());

		} else {
			return new OriginalVersion<Locatable>(uid, null, data, codedText,
					audit1, lr, null, null, null, TestTerminologyService.getInstance());
		}
	}

	protected ImportedVersion<Locatable> importedVersion(Locatable data,
		...
		AuditDetails audit = new AuditDetails("1.32.4.15", pi, new DvDateTime(
				...
				.getInstance());
		return new ImportedVersion<Locatable>(originalVersion(data, isMerged,
