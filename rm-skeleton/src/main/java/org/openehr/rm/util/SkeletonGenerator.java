...
public class SkeletonGenerator {
	...
	private SkeletonGenerator() throws Exception {
		...
		openEHRTerminology = terminologyService
	...
	public void loadTermMapFromClasspath(String filename) throws Exception {
		InputStream input = SkeletonGenerator.class.getClassLoader()
				.getResourceAsStream(filename);
		if (input == null)
			throw new IllegalArgumentException("Can not open filename="
					+ filename);
		termMap.clear();
		termMap.addAll(input);
	}
	...
	/**
	 * Create RM object tree based on given archetype with a default minimum
	 * strategy
	 * 
	 * @param archetype
	 * @return
	...
	public Object create(Archetype archetype, GenerationStrategy strategy)
			throws Exception {
	...
	/**
	 * Create RM object tree based on given main the archetype and associated
	 * archetypes in the archetypeMap
	 * 
	...
	public Object create(Archetype archetype,
		...
		return create(archetype, null, archetypeMap, null,
	...
	public Object create(Archetype archetype, String templateId,
			...
			Map<String, Object> extraValues, GenerationStrategy strategy)
			throws Exception {
	...
	public Object createComplexObject(CComplexObject ccobj,
				...
				if (cattr.isAllowed()
						&& (GenerationStrategy.MAXIMUM.equals(strategy)
								|| GenerationStrategy.MAXIMUM_EMPTY
										.equals(strategy) || (GenerationStrategy.MINIMUM
								.equals(strategy) && cattr.isRequired()))) {
					Object attrValue;
		...
		if ("DV_TEXT".equals(rmTypeName)) {
			...
			if (valueMap.get(DESCRIPTION) == null) {
				valueMap.put(DESCRIPTION, new ItemTree("at0001", new DvText(
						"tree"), null));
			}
			...
			if (GenerationStrategy.MAXIMUM_EMPTY.equals(strategy)
					&& (("INPUT".equals(ccobj.getAnnotation()) || ccobj
							.isAnyAllowed()))) {
				...
				// should not be necessary now when the AOM from flattener is
				// fixed
			...
			/*
			 * if( ! valueMap.containsKey("width")) { valueMap.put("width", new
			 * DvDuration("P1d")); } if( ! valueMap.containsKey("time")) {
			 * valueMap.put("time", new DvDateTime()); }
			 */
			return null;
		...
		} else if ("PERSON".equals(rmTypeName)) {
			addEntryValues(valueMap, archetype);
		}
	...
	public Object createAttribute(CAttribute cattribute, Archetype archetype,
		...
		if (cattribute instanceof CSingleAttribute) {
			...
			if (children != null && children.size() > 0) {
				...
				return createObject(cobj, archetype, archetypeMap, extraValues,
						strategy);
			} else {
			...
			for (CObject cobj : children) {
				...
				if (cobj.isAllowed()
						&& (GenerationStrategy.MAXIMUM.equals(strategy)
								|| GenerationStrategy.MAXIMUM_EMPTY
										.equals(strategy) || (GenerationStrategy.MINIMUM
								.equals(strategy) && cobj.isRequired()))) {
	...
	public Object createObject(CObject cobj, Archetype archetype,
		...
		if (cobj instanceof CComplexObject) {
			...
			return createComplexObject((CComplexObject) cobj, archetype, null,
					archetypeMap, extraValues, strategy);
	 ...
	 * @throws Exception
	 */
	String retrieveArchetypeTermText(String nodeId, Archetype archetype)
			throws Exception {
		String language = archetype.getOriginalLanguage().getCodeString();
		ArchetypeTerm term = archetype.getOntology().termDefinition(language,
				nodeId);
		if (term == null) {
